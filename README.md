...
| `pyproject.toml`       | Package metadata, the pixi environment, and tool configuration                    |
...
The Dagster jobs are defined in `usage_metrics.etl`.
Assets that extract, transform and load each datasource live in the `raw`, `core` and `out` modules.
`all_partitioned_metrics_etl` and `all_nonpartitioned_metrics_etl` run every source;
there is also a per-source job (e.g. `s3_metrics_etl`, `eel_hole_metrics_etl`).
