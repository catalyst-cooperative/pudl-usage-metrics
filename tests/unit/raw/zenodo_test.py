...
def test_filter_blobs_keeps_in_range_json_only(make_client, partition_context):
    ...
    kept = {
        blob.name for blob in ext.filter_blobs(partition_context("2024-01-01"), listed)
    }
...
def test_extract_keeps_per_file_metrics_date(
    download_dir, run_extract, partition_context
):
    ...
    df = run_extract(ZenodoExtractor(), partition_context("2024-01-01"), [a, b])
